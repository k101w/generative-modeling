...
def compute_discriminator_loss(
    ...
    ##################################################################
    grad_interp = torch.autograd.grad(inputs=interp, outputs=discrim_interp, grad_outputs=torch.ones_like(discrim_interp),create_graph=True,
            retain_graph=True)[0]
    grad_norm = grad_interp.norm(p=2,dim=1)
    loss_pt1 = torch.mean(discrim_fake) - torch.mean(discrim_real)
    loss_pt2 = lamb *  torch.mean((grad_norm - 1)**2)
    loss = loss_pt1 + loss_pt2
    ##################################################################
    #                          END OF YOUR CODE                      #
    ##################################################################
