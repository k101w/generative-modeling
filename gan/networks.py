...
class DownSampleConv2D(torch.jit.ScriptModule):
    ...
    def forward(self, x):
        ...
        x = x.reshape(b,c,int(self.downscale_ratio**2),h//self.downscale_ratio ,w//self.downscale_ratio).permute(2,0,1,3,4)
        x = x.mean(0)
        x = self.conv(x)
        return x
...
class ResBlockUp(torch.jit.ScriptModule):
    ...
    def __init__(self, input_channels, kernel_size=3, n_filters=128):
        ...
        self.upsample_residual = UpSampleConv2D(input_channels, kernel_size=3, n_filters=n_filters, padding=1)
...
class ResBlockDown(torch.jit.ScriptModule):
    ...
    def __init__(self, input_channels, kernel_size=3, n_filters=128):
        ...
        self.downsample_residual = DownSampleConv2D(input_channels, kernel_size=kernel_size, n_filters=n_filters, padding=1)
        ##################################################################
        #                          END OF YOUR CODE                      #
        ##################################################################
    ...
    def forward(self, x):
        ...
        ##################################################################
        x_f = self.layers(x)
        x_r = self.downsample_residual(x)
        return x_f + x_r
        ##################################################################
        #                          END OF YOUR CODE                      #
        ##################################################################
...
class ResBlock(torch.jit.ScriptModule):
    ...
    def __init__(self, input_channels, kernel_size=3, n_filters=128):
        ...
        self.res = nn.Conv2d(input_channels, n_filters, kernel_size=3, stride=1, padding=1)
        ##################################################################
        #                          END OF YOUR CODE                      #
        ##################################################################
...
class Generator(torch.jit.ScriptModule):
    ...
    def forward_given_samples(self, z):
        ...
        ##################################################################
        b = z.shape[0]
        z =  self.dense(z).reshape(b,-1,self.starting_image_size,self.starting_image_size)
        z = self.layers(z)
        return z
        ##################################################################
        #                          END OF YOUR CODE                      #
